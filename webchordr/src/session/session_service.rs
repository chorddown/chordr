use libchordr::prelude::{Credentials, Username, Password, User};
...
impl SessionService {
    ...
    pub async fn try_login(&mut self, credentials: &Credentials) -> Result<Session, WebError> {
        match self.perform_login(credentials).await {
            Ok(session) => {
                let _ = self.set_credentials_in_session_storage(credentials);
                Ok(session)
            }
            Err(e) => Err(e)
        }
    ...
    pub async fn try_session(&self) -> Result<Session, WebError> {
    ...
    fn set_credentials_in_session_storage(&mut self, credentials: &Credentials) -> Result<(), WebError> {
        self.session_storage.set_item(STORAGE_KEY_PASSWORD, credentials.password().to_string())?;
        self.session_storage.set_item(STORAGE_KEY_USERNAME, credentials.username().to_string())?;
