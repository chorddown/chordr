...
pub struct SongList<S: SongIdTrait>(Vec<S>);

impl<S: SongIdTrait> SongList<S> {
    pub fn new() -> Self {
        Self { 0: Vec::new() }
    }
...
impl<S: SongIdTrait> SongListTrait for SongList<S> {
    type Item = S;
    ...
    // }
    //
    // fn get(&self, song_id: SongId) -> Option<&S> {
    ...
    fn replace(&mut self, song: S) -> Result<()> {
        let song_id = song.id();
    ...
    // fn remove(&mut self, song: &S) -> Result<()> {
...
impl<S: SongIdTrait> ops::Index<usize> for SongList<S> {
    type Output = S;
