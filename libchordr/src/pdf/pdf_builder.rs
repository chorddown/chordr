...
impl PdfBuilder {
    pub fn new(coordinates: Coordinates, styles: Styles, doc: PdfDocumentReference, layer: PdfLayerReference) -> Self {
        Self {
            coordinates,
            styles,
            layer,
...
//                let text = format!("{:?}", node);
                ...
                self.layer.use_text(text, style.font_size, self.coordinates.x, self.coordinates.y, &style.font);
...
//                    .build()
            ...
            Token::Literal(c) => {
                let style = self.styles.chord().unwrap();
                self.layer.use_text(c, style.font_size, self.coordinates.x, self.coordinates.y, &style.font);
...
mod tests {
    ...
    use crate::pdf::styles::{Style, Styles, PageSize};
